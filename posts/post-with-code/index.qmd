...
The anova performed above tells us to go with the model with less variables, or in other words our model without the interaction term. The second test performed gives us the variation inflation factor of our two predictor variables from our original model. I ran this test because it allows us to know if there is collinearity between our predictor variables. Because both scores our below 5 this means that we shouldn't have to worry about mutlicollinearity. This further supports the evidence that we probably do not need to add a interaction term.
