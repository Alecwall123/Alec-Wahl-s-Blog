...
Above is a mortality table created analyze the hospital outcome based on Sex. A hospital outcome of 1 indicates the patient survived. Sex equaling 1 indicates a male. As seen when analyzed both females and males have fairly similar survival rates at 92.15% for females and 93.18% for males.
...
Graphed above we have the Hospital Outcomes for patients as age increases from 0-100. When it equals true that means we had a negative outcome, as seen above there is definitely an increase in negative outcomes as patients get older.
...
```
In code block above, we turn our continuous variables to factors so we can run log function correctly as they are binomial anyways.
...
```
Above we chose to run a generalize linear model instead of a regular lm this is due to our output being of a binomial nature. As seen above, of our predictor variables, all have p values below a significance level of .05 besides episode number. In some ways at a biological level this makes sense, due to the nature of sepsis being a infection in the blood stream from any number of different infections. Previous instances of sepsis would not necessary mean someone is at risk of further having Sepsis.
...
Our final decision then will be to as seen above, remove episode number and create a new data.
